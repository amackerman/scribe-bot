...
const createCommand = async (interaction, client) => {
    ...
    const userMessages = [...messages.values()].filter(message => message.author.id !== client.user.id).reverse();
    ...
    try {
        ...
        if (fs.existsSync(DOC_STORAGE_PATH)) {
            currentStorage = JSON.parse(fs.readFileSync(DOC_STORAGE_PATH, 'utf8'));
        }
        ...
        fs.writeFileSync(DOC_STORAGE_PATH, JSON.stringify(currentStorage, null, 4));
        ...
        interaction.editReply(`Failed to create Google Doc named '${docName}'.`);
    }
}
